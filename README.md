"""
╔══════════════════════════════════════════════════════════════════════════╗
║               🔋  Battery NEXT Power — EV PowerShare                    ║
║               Smart Energy Management Dashboard                          ║
╚══════════════════════════════════════════════════════════════════════════╝

What is this?
─────────────
This is a desktop energy dashboard built with Python.

The idea behind it is simple: what if your home could think smarter
about energy? You plug in your appliances, pick where your electricity
comes from (solar panels, EV batteries, green-certified grid, etc.),
and the app shows you — month by month — how much energy you're using,
what it's costing you, and even gives a rough AI-style prediction of
where things are heading.

It was built as part of the Battery NEXT Power project, which explores
how second-life EV batteries (like those from Stellantis vehicles) can
be reused to store solar energy and power homes in a smarter, cheaper,
and greener way.

Who is it for?
──────────────
Anyone curious about their home energy use — homeowners, students,
engineers, or anyone interested in clean energy and EV technology.
No technical background needed to run it.

What can it do?
───────────────
  📊  Dashboard     — Monthly consumption bar chart + appliance share pie chart
  📈  Usage Chart   — Annual energy trend with rolling average
  💶  Cost Analysis — Monthly cost breakdown including VAT (22%)
  🏠  Appliances    — Adjust per-appliance daily usage with sliders
  🔋  KPI Cards     — Live summaries: total usage, cost, CO2 saved, AI prediction
  📄  Export        — Save a full text report to disk with one click

How to run it?
──────────────
  1. Make sure Python 3.8+ is installed
  2. Install the only extra library needed:
  ...
  That's it. A window will open — no browser, no internet, no account needed.

Project context
───────────────
  Course    : BiGin 2026 — Business & Innovation
  Topic     : Community Energy (CER) + Second-Life EV Batteries
  Developer : Rohina Bawer
...
# ──────────────────────────────────────────────────────────────────────────
#  COLOR PALETTE  (purple-dark theme, green accents)
# ──────────────────────────────────────────────────────────────────────────
C_BG_DARK   = "#1A0A2E"   # deepest background
C_BG_MID    = "#2D1B5E"   # sidebar / cards
C_BG_LIGHT  = "#3D2578"   # inner card highlight
C_ACCENT1   = "#7B2FBE"   # primary purple
C_ACCENT2   = "#9D4EDD"   # lighter purple
C_GREEN     = "#00C853"   # positive / success
C_GREEN_D   = "#00953D"   # button hover
C_WHITE     = "#F0E6FF"   # text primary
C_GREY      = "#B0A0CC"   # text secondary
C_YELLOW    = "#FFD600"   # highlights / warnings
C_RED       = "#FF4B4B"   # alerts / peak values
C_CHART_BG  = "#150826"   # matplotlib figure background


# ──────────────────────────────────────────────────────────────────────────
#  ELECTRICITY SOURCE TYPES  (price in € / kWh)
# ──────────────────────────────────────────────────────────────────────────
ELECTRICITY_TYPES = {
...
# ──────────────────────────────────────────────────────────────────────────
#  HOME APPLIANCES  (default daily usage in kWh)
# ──────────────────────────────────────────────────────────────────────────
APPLIANCES = {
    "Refrigerator":    1.5,
    "Washing Machine": 1.2,
...
class HomeEnergyApp(tk.Tk):
    """Main tkinter application window for the EV PowerShare dashboard."""
    ...
    def __init__(self):
        ...
        # Simulated monthly consumption data (kWh) — replace with real data as needed
    ...
    def _build_sidebar(self):
        ...
        self.kpi_co2           = self._kpi_card(kpi_frame, "CO₂ Saved",     "—  kg")
    ...
    def _section_header(self, parent, text):
        """Renders a small green section label in the sidebar."""
        tk.Label(parent, text=text, bg=C_BG_MID, fg=C_GREEN,
                 font=("Helvetica", 10, "bold"), anchor="w"
                 ).pack(fill="x", padx=14, pady=(10, 2))

    def _kpi_card(self, parent, label, value):
        """Renders a single KPI card with a label and a live value."""
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  TAB 1 — DASHBOARD  (bar chart + pie chart)
    # ──────────────────────────────────────────────────────────────────────
    ...
    def _draw_dashboard(self):
        ...
        # Color bars by consumption level: green → yellow → red
        ...
        ax_b.set_title("Monthly Consumption (kWh)", color=C_WHITE,
                       fontsize=11, pad=10)
        ...
        ax_p.set_title("Appliance Share (%)", color=C_WHITE, fontsize=11, pad=10)
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  TAB 2 — USAGE CHART  (line chart with rolling average)
    # ──────────────────────────────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  TAB 3 — COST ANALYSIS  (stacked bar: base cost + VAT)
    # ──────────────────────────────────────────────────────────────────────
    ...
    def _draw_cost_chart(self):
        ...
        tax   = [c * 0.22  for c in costs]           # Italian VAT: 22 %
        ...
        ax.tick_params(axis="y", colors=C_GREY, labelsize=9)
        ax.set_title(
            f"Monthly Cost Analysis  |  Rate: €{price:.2f}/kWh  |  "
            f"Payment: {self.pay_method.get().split()[1]}",
            color=C_WHITE, fontsize=10, pad=10)
        ax.set_ylabel("EUR (€)", color=C_GREY, fontsize=10)
        ax.yaxis.grid(True, color=C_BG_LIGHT, linestyle="--", linewidth=0.5)
        ax.set_axisbelow(True)
        for spine in ax.spines.values():
            spine.set_edgecolor(C_BG_LIGHT)
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  TAB 4 — APPLIANCES  (interactive sliders)
    # ──────────────────────────────────────────────────────────────────────
    def _build_appliances_tab(self, parent):
        ...
        scrollbar = ttk.Scrollbar(canvas_frame, orient="vertical", command=cv.yview)
        ...
        for i, (name, var) in enumerate(self.appliance_vars.items()):
            ...
            tk.Label(row, text=f"{APPLIANCE_ICONS[i]}  {name}", bg=C_BG_MID,
    ...
    def _update_appl_label(self, name):
        """Called live as a slider moves — updates its label and progress bar."""
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  KPI LOGIC  (all five cards)
    # ──────────────────────────────────────────────────────────────────────
    def _refresh_all(self):
        """Master refresh: update price badge, all KPIs, and all charts."""
    ...
    def _update_price_badge(self):
        info = ELECTRICITY_TYPES[self.elec_type.get()]
        self.price_badge.config(text=f"{info['icon']}  €{info['price']:.2f} / kWh")

    def _update_kpis(self):
        price  = ELECTRICITY_TYPES[self.elec_type.get()]["price"]
        period = self.period.get()
        total  = sum(self.monthly_usage)     # annual total (kWh)
        ...
        # KPI 1 — Total usage for the selected period
        self.kpi_usage.config(text=f"{total * multiplier:,.1f} kWh")

        # KPI 2 — Total cost for the selected period
        self.kpi_cost.config(text=f"€ {cost * multiplier:,.1f}")

        # KPI 3 — Average monthly consumption
        self.kpi_avg.config(text=f"{total / 12:,.1f} kWh")

        # KPI 4 — Estimated CO₂ saved vs. average Italian grid (0.233 kg/kWh)
        self.kpi_co2.config(text=f"{total * 0.233:,.0f} kg")

        # KPI 5 — Simple AI prediction: years until consumption grows 20%
        #          based on the coefficient of variation (CV) of monthly data
        avg_monthly = total / 12
        variance    = sum((m - avg_monthly) ** 2 for m in self.monthly_usage) / 12
        cv          = (variance ** 0.5) / avg_monthly if avg_monthly else 0
        if cv > 0:
            years = round(0.20 / (cv * 0.05), 1)
            years = max(1.0, min(years, 30.0))
        else:
            years = 10.0
        self.kpi_ai_prediction.config(text=f"{years:.1f} yrs")

    def _randomize_data(self):
        """Generate new random monthly usage — useful for demos."""
        self.monthly_usage = [round(random.uniform(180, 480), 1) for _ in range(12)]
        self._refresh_all()

    # ──────────────────────────────────────────────────────────────────────
    #  EXPORT REPORT  (UTF-8 text file, auto-opened after save)
    # ──────────────────────────────────────────────────────────────────────
    def _export_report(self):
        ...
        # Recompute AI prediction (same formula as KPI 5)
        avg_monthly = total / 12
        variance    = sum((m - avg_monthly) ** 2 for m in self.monthly_usage) / 12
        cv          = (variance ** 0.5) / avg_monthly if avg_monthly else 0
        years_pred  = round(0.20 / (cv * 0.05), 1) if cv > 0 else 10.0
        ...
        for month, usage in zip(MONTHS, self.monthly_usage):
            lines.append(
                f"    {month:>3}  ->  {usage:6.1f} kWh  |  EUR {usage * price:6.2f}"
            )
        lines += [
            "-" * 52,
            ...
            f"  AI PREDICTION       : {years_pred:.1f} years to +20% growth",
            "=" * 52,
        ]

        report_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "energy_report.txt"
        )
    ...
    # ──────────────────────────────────────────────────────────────────────
    #  LIVE CLOCK  (top-right corner, updates every second)
    # ──────────────────────────────────────────────────────────────────────
